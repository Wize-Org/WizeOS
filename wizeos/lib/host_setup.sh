...
setup_host() {
  ...
  apt-get update
  apt-get install -y ca-certificates curl gnupg sudo repo git openssh-client python3 zip unzip rsync diffutils fontconfig fonts-dejavu-core hostname openssl gperf gcc-multilib libc6-dev-i386 build-essential lftp
